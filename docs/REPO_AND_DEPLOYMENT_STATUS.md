...
- ⏸️ **3-C**: 旧Vercelプロジェクト `minpaku-sim` の整理 → **未実施**（数日様子見してから判断）
- ⏸️ **3-D**: 旧GitHubリポジトリ `0ne9uy/minpaku-sim` のアーカイブ → **未実施**（同上）
...
### 旧Vercelプロジェクト `minpaku-sim` の処遇

新URLが安定稼働していることを数日〜数週間確認したのち、以下のいずれかを実施:

| 案 | 内容 | 影響 |
|---|---|---|
| C1: 放置 | 何もしない | 旧URLは古いコードを表示し続ける（将来の混乱要因） |
| C2: Git連携を切る | Settings → Git → Disconnect | 新規デプロイ停止。既存URLは生きたまま |
| C3: プロジェクト削除 | Settings → Advanced → Delete Project | **取り消し不可**。全URL消滅 |

### 旧GitHubリポジトリ `0ne9uy/minpaku-sim` の処遇

Settings → Danger Zone → **Archive this repository** で読み取り専用化（解除可能）が無難。
...
|------|----------|
| 2026-05-11 | 調査・初版作成 |
| 2026-05-12 | 整理作業を実施（新規Vercelプロジェクト作成、fixブランチ削除）。本ドキュメントを最新化 |
